    ...
    for i in range(len(vsrl_data_all)):
        ...
        for j in range(num_roles):
            ...
            if v_i['include'][j] != []:
                ...
                bbox_j[:, 4 * j:4 * j + 4] = xyhw_to_xyxy(
                    np.vstack([np.array(a['bbox']).reshape((1, 4)) for a in anns]))
...
def attach_gt_boxes(vsrl_data, coco):
    ...
    """
    # get v_coco_all boxes
    # ravel:N*1-->N
    # vsrl_data['ann_id] --> annotations_id --> anns
    ...
    for i in range(1, vsrl_data['role_object_id'].shape[1]):
    ...
    # 3. add image file name
    images = coco.loadImgs(vsrl_data['image_id'].ravel().tolist())
    file_name = np.array([a['file_name'] for a in images])
    vsrl_data['file_name'] = file_name
